...
export class ConversationsRepository {
  ...
  async createOrFind(user1Id: string, user2Id: string, originPlaybackEventId: string | null): Promise<Conversation> {
    // Determine strict order to avoid duplicates (user1 < user2)
  ...
  async findByUserId(userId: string): Promise<any[]> {
    // This query lists the user's inbox conversations
    // The "lastMessage" and "unreadCount" logic would require matching via MongoDB, 
    // or through an aggregator service later. For now we respond with raw postgres inbox data joined with opposite user profile.
    const query = `
      SELECT 
...
export class ConversationsController {
  ...
  async getConversations(@Req() req: any) {
    // This lists standard conversations
    ...
    return items.map(item => ({
      ...
      lastMessage: null, // Will be aggregated by messaging service or frontend later
      unreadCount: 0,
