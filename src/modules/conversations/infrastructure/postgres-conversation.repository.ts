...
export class PostgresConversationRepository implements ConversationRepository {
  ...
  async createOrFind(user1Id: string, user2Id: string): Promise<Conversation> {
    ...
    if (res.rows.length > 0) {
      const r = res.rows[0];
      return new Conversation(r.id, r.user1_id, r.user2_id, r.created_at, r.updated_at);
    }
    const insertQuery = `
      ...
      RETURNING *;
    `;
    const inserted = await this.pool.query(insertQuery, [uuidv4(), u1, u2, new Date(), new Date()]);
